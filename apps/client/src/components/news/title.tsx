...
export default function NewsTitle({
  ...
  if (variant === "dark") {
    return (
      <div className={cn(
        "flex items-center gap-x-4 max-md:gap-1 my-10 max-md:my-5 uppercase bg-[#ff8400] px-6 py-3 rounded-[5px]",
        className
      )}>
        <h3 className="mr-auto text-white text-3xl font-bold max-md:text-base">
          {title}
        </h3>
        {button && href && (
          <Link
    ...
    <div className="flex items-center gap-x-4 max-md:gap-1 my-10 max-md:my-5 uppercase">
      ...
      {button && href && (
        <Link
          lang={lang}
          href={href}
          className="flex items-center text-[#ff8400] font-bold"
        >
          {button}
          <ChevronRight className="w-5 h-5" />
        </Link>
